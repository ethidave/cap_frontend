...
import { Mail, Lock, Eye, EyeOff, User, TrendingUp, ArrowRight, ArrowLeft } from "lucide-react";
...
export default function SignUpPage() {
    ...
    const handleSignup = async (e: React.FormEvent) => {
        ...
        try {
            ...
            toast("success", "Account Created", res.message || "Please verify your email.");
    ...
    return (
        <main className="min-h-screen bg-[#020617] flex relative">
            ...
            <div className="w-full lg:w-1/2 flex flex-col relative min-h-screen overflow-y-auto">
                ...
                <div className="flex-1 flex items-center justify-center px-6 py-24 md:px-12">
                    <motion.div
                    ...
                    >
                        <div className="mb-6 md:mb-10 text-left">
                            <h1 className="text-3xl md:text-4xl font-black text-white mb-3 tracking-tight">Create Account</h1>
                            <p className="text-slate-400 font-medium text-sm md:text-base">
                                Already have an account?{" "}
                                <Link href="/signin" className="text-[#00FFA3] hover:underline decoration-2 underline-offset-4">Log in</Link>
                            </p>
                        </div>
                        ...
                        <form onSubmit={handleSignup} className="space-y-5 mb-8">
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-4">
                                <div>
                                    ...
                                    <div className="relative group">
                                        <User className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-600 group-focus-within:text-[#00FFA3] transition-colors z-10" />
                                        <input
                                            required
                                ...
                                <div>
                                    ...
                                    <input
                                        required
                                        className="input-field h-14 bg-white/[0.02] border-white/5 focus:bg-white/[0.04] px-6"
                                        placeholder="Doe"
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-4">
                                ...
                                <div>
                                    <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2 px-1">Date of Birth</label>
                                    <input
                                        required
                                        type="date"
                                        className="input-field h-14 bg-white/[0.02] border-white/5 focus:bg-white/[0.04] px-5 text-slate-400"
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 md:gap-4">
                                ...
                                <div>
                                    <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-2 px-1">Confirm Key</label>
                                    <div className="relative group">
                                        <Lock className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-600 group-focus-within:text-[#00FFA3] transition-colors z-10 md:hidden" />
                                        <input
                                            required
                                            className="input-field h-14 bg-white/[0.02] border-white/5 focus:bg-white/[0.04] px-6"
                                            placeholder="••••••••"
                            ...
                            <div className="flex items-center gap-3 mb-10 overflow-hidden">
                                <input required type="checkbox" id="terms" className="w-5 h-5 rounded border-white/10 bg-white/5 accent-[#00FFA3] cursor-pointer" />
                                <label htmlFor="terms" className="text-sm text-slate-400 cursor-pointer select-none">
                                    I agree to the <Link href="#" className="text-[#00FFA3] font-bold hover:underline">Terms & Conditions</Link>
                                </label>
                            </div>

                            <button
                                disabled={isLoading}
                                className="btn-primary w-full h-14 shadow-[0_0_20px_rgba(0,255,163,0.2)] mb-8 flex items-center justify-center gap-2"
                            >
                                {isLoading ? <div className="w-5 h-5 border-2 border-[#020617] border-t-transparent rounded-full animate-spin" /> : "Create Account"}
                        ...
                        <div className="relative mb-8">
                            ...
                            <div className="relative flex justify-center text-[10px] uppercase font-black tracking-widest">
                                <span className="bg-[#020617] px-4 text-slate-500">Or Register with</span>
                            </div>
                        </div>

                        <button
                            type="button"
                            onClick={() => window.location.href = (process.env.NEXT_PUBLIC_API_URL || "https://api.captradepro.com") + "/api/auth/google"}
